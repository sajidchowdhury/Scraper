...
- [ ] Define a clear folder structure:
  ...
  └── README.md
  ```
- [ ] Add a real `start` script in `package.json` (e.g., `"start": "node src/index.js"`).
- [ ] Add `.gitignore` covering `node_modules/`, `data/`, `logs/`, `.env`, OS files (`desktop.ini`, `.DS_Store`).
- [ ] Remove the committed `desktop.ini` (it's a Windows folder-customization artifact that should never be in version control).
- [ ] Create `.env.example` documenting every env var the script will use (added in Phase 1.1).
- [ ] Decide Node version (e.g., Node 20 LTS) and document it.
- [ ] Move current `main.js` logic into `src/index.js` as the starting point; archive `test.js` into a `scripts/` folder or delete it.
...
- [ ] Implement CLI arg parsing (use `commander`, `yargs`, or hand-rolled `process.argv` parser).
  - Example: `npm start -- --query "Restaurant" --location "Toronto" --maxResults 50`
- [ ] Implement env-var fallbacks (loaded via `dotenv` — already a dependency).
- [ ] Implement `.env.example` with documented keys:
  ```
  ...
  OUTPUT_DIR=./data
  ```
- [ ] Validate inputs on startup — fail fast with a clear message if `query` or `location` is missing.
- [ ] Print the resolved config at startup so the user sees exactly what will run.
...
### Task checklist
- [ ] Extract browser launch into `src/browser.js` with a function that returns `{ browser, page }` and respects config (headless, slowMo, viewport from Phase 1.1).
- [ ] Extract Maps navigation + search into `src/search.js`.
- [ ] In `src/index.js`, implement a clean lifecycle:
  ```
  launch → navigate → search → [paginate] → [extract] → [export] → close
  ```
- [ ] **Remove the `await new Promise(() => {})` line** — replace with the actual work pipeline, then `await browser.close()`.
- [ ] Wrap everything in a `try/finally` so the browser **always** closes, even on error.
- [ ] Add a global timeout (configurable, default 5 minutes) that kills the run if it hangs.
- [ ] Add a `SIGINT` (Ctrl-C) handler that gracefully closes the browser before exiting.
- [ ] Verify the search input selector (`input#searchboxinput`) and Enter-key submission still work; if Google changed the DOM, update with a documented fallback selector.
...
### Task checklist
- [ ] Identify the scrollable results container (`div[role="feed"]` — already detected in current code).
- [ ] Implement a scroll loop that:
...
- [ ] Track scroll progress in logs (e.g., "Scrolled: 20 → 40 → 60 → ...").
- [ ] Handle the "Arenas" / "scroll-then-click-Show-more" edge case if it appears for very large result sets.
- [ ] Detect and break out of any "restart search" / "back to results" loops that Maps sometimes triggers.
...
### Task checklist
- [ ] Define the **canonical field list** for Phase 1 (list-view extractable):
  | Field | Source | Notes |
  |---|---|---|
...
- [ ] Implement extraction in `src/extract.js` as a function that takes the page and returns an array of business objects.
- [ ] Use **multiple fallback selectors** per field — Google changes the DOM frequently; if the primary selector misses, try alternates before giving up.
- [ ] For each field, log a **per-field extraction rate** at the end (e.g., "phone: 198/200 = 99%"). Drops below 80% trigger a warning (early signal of a DOM change).
- [ ] Normalize data on extraction:
...
- [ ] Handle "permanently closed" and "temporarily closed" businesses — extract the status flag, don't skip them (clients want to know who closed).
- [ ] Handle sponsored/ad results — flag them with `is_sponsored: true` so they're not confused with organic results.
...
### Task checklist
- [ ] Make detail scraping **toggleable** via config (`--deepScrape true|false`, default `false` for Phase 1 to keep runs fast).
- [ ] For each business (when `deepScrape` is on):
...
- [ ] Add a per-detail delay (configurable, default 1-3s randomized) to avoid hammering Google.
- [ ] Handle detail-panel load failures gracefully — log and continue to next business (don't fail the whole run).
- [ ] Track detail-scrape success rate in logs.
...
### Task checklist
- [ ] Use the already-installed `csv-writer` package.
- [ ] Define a **stable column order** matching the canonical field list from Phase 1.4 (plus detail fields if Phase 1.5 is on).
- [ ] Handle CSV escaping correctly:
...
- [ ] Use **UTF-8 with BOM** so Excel opens non-Latin characters (Bengali, Arabic, emoji in business names) without garbling.
- [ ] Auto-generate the output filename if not specified:
...
- [ ] Print the absolute output path at the end of the run so the user knows exactly where the file is.
- [ ] Also write a **JSON export** alongside the CSV (same data, nested fields preserved) — costs almost nothing, huge value for technical clients.
- [ ] Write a small **run summary** file (`.json`) next to the CSV capturing: query, location, total results, extraction rates per field, start/end time, duration, output filenames. Useful for the operator's own QA.
...
### Task checklist
- [ ] Implement **retry with backoff** for transient operations (page.goto, selector waits):
  - 3 attempts
  - Exponential backoff: 1s → 2s → 4s
  - On final failure, log and skip (don't crash the whole run).
- [ ] Implement a **state checkpoint** mechanism:
...
- [ ] Implement **per-business error isolation** — if extraction fails for one business, log it and continue to the next. The run only fails if a systemic error occurs (browser crash, total network loss).
- [ ] Track and log:
  - Total businesses found
  - Successfully extracted
  - Failed (with reasons)
  - Skipped (already in checkpoint)
- [ ] Add a `--resume` flag and a `--fresh` flag (force-start, ignore checkpoint).
...
### Task checklist
- [ ] Implement structured logging in `src/logger.js` (use `winston`, `pino`, or a thin custom wrapper).
- [ ] Log to **two sinks** simultaneously:
...
- [ ] Print a **clean, scannable console summary** at the end:
  ```
  ========================================
  Run complete
...
### Task checklist
- [ ] Implement `--help` output that lists every flag with a one-line description and a usage example.
- [ ] Implement `--version` (read from `package.json`).
- [ ] Implement `--dryRun` — do everything except write the CSV (useful for testing selectors and pagination).
- [ ] Implement `--limit N` (alias for `--maxResults`) for quick test runs.
- [ ] Implement `--headless` / `--headed` overrides (force a mode regardless of `.env`).
- [ ] Implement `--verbose` (alias for `--logLevel debug`).
...
- [ ] Validate all CLI inputs and print friendly errors (not stack traces) for bad input.
- [ ] Exit codes: `0` success, `1` partial success (some failures), `2` config error, `3` runtime error.
...
- [ ] Add inline code comments explaining **why**, not **what** (especially around selectors and scroll logic — the fragile parts).
